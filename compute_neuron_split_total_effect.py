...
# def filtered_mean(df, column_name, profession_stereotypicality, direction="None"):
#     # If we filter to a single direction, calculate stereotypicality
#     if direction == 'male':
#         return df[(df['definitional'] < 0.75) & (df['stereotypicality'] < 0)][column_name].mean()
#     elif direction == 'female':
#         return df[(df['definitional'] < 0.75) & (df['stereotypicality'] > 0)][column_name].mean()
#     # print(df[['base_string', 'she_total_effect', 'he_total_effect', 'total_effect']])
#     # exit()
...
def main(folder_name="results/20191114_neuron_intervention/",
    ...
    for path in woman_files:
        ...
        df['profession'] = df['base_string'].apply(
            lambda s: s.split()[1])
        df['definitional'] = df['profession'].apply(
            get_definitionality)
        df['stereotypicality'] = df['profession'].apply(
            get_stereotypicality)
        '''
        FILTERING
        '''
        ...
        df['man_he_she_effect'] = df['candidate1_alt1_prob'] / df['candidate2_alt1_prob']
        df['woman_she_he_effect'] = df['candidate2_alt2_prob'] / df['candidate1_alt2_prob']
        # Compute profession effect
        df['base_he_she_effect'] = df['candidate1_base_prob'] / df['candidate2_base_prob']
        df['base_she_he_effect'] = df['candidate2_base_prob'] / df['candidate1_base_prob']
        ...
        female_mean_model = df[df['base_she_he_effect'] > 1.]['he_she_total_effect'].values
        female_means_model.extend(female_mean_model)

        male_mean_model = df[df['base_he_she_effect'] > 1.]['she_he_total_effect'].values
        male_means_model.extend(male_mean_model)

        # (2) Filter by stereotype
        female_mean_def = df[df['stereotypicality'] < 0.]['he_she_total_effect'].values
        female_means_def.extend(female_mean_def)

        male_mean_def = df[df['stereotypicality'] > 0.]['she_he_total_effect'].values
        male_means_def.extend(male_mean_def)

    # print("The total effect of this model is {:.3f}".format(np.mean(means)-1))
    print("The total (female profession (model) -> man) effect of this model is {:.3f}".format(
        np.mean(male_means_model)-1))
    print("The total (male profession (model) -> woman) effect of this model is {:.3f}".format(
        np.mean(female_means_model)-1))
    print("The combined effect is {:.3f}".format(
        np.mean(female_means_model + male_means_model)-1))

    print("The total (female profession (def) -> man) effect of this model is {:.3f}".format(
        np.mean(male_means_def)-1))
    print("The total (male profession (def) -> woman) effect of this model is {:.3f}".format(
